defmodule CreatePLayers do
  ...
  def create_struct(player) do
    ...
    |> ChallengeNfl.Repo.insert!
  end
  defp build_total_yards_field(total_yards) do
  ...
  defp build_longest_rush(lng) do
    ( lng |> is_integer() && lng ) ||
    lng
    |> String.replace("T", "")
    |> String.to_integer()
  ...
  defp build_longest_rush_with_touch_down(lng) do
    if is_integer(lng) do
      false
    else
      String.contains?(lng, "T")
    end
  end
end
